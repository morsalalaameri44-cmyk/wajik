    ...
    container.innerHTML = `
        ...
        <div style="max-width: 900px; margin: 0 auto; padding-bottom: 30px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:25px;">
                ...
                <button class="add-btn" onclick="openAddStoreModal()"><i class="fa-solid fa-plus"></i> إضافة متجر جديد</button>
            </div>
...
function renderStoresList(stores, items) {
    ...
    if(stores.length === 0) {
        container.innerHTML = `<div style="text-align:center; color:#64748b; padding:50px; background:#fff; border-radius:16px; border:1px solid #e2e8f0;"><h3>لا توجد متاجر مسجلة حالياً</h3></div>`;
        return;
    }

    let html = '';
    stores.forEach(store => {
        // قراءة دقيقة لأعمدة قاعدة البيانات لتطبيق "واجيك"
        const storeName = store.store_name || store.name || store.title || 'متجر غير محدد';
        const storeCategory = store.category || store.store_category || store.type || 'عام';
        const storePhone = store.phone || store.store_phone || 'بدون رقم';

        // تحديد أيقونة تناسب القسم
        ...
        else if(storeCategory.includes('مكتب')) iconClass = 'fa-book';
        else if(storeCategory.includes('مطعم') || storeCategory.includes('وجبات')) iconClass = 'fa-utensils';

        const storeItems = items.filter(i => i.store_id === store.id);
        
        let itemsHtml = '';
        if(storeItems.length === 0) {
            itemsHtml = `<p style="color:#94a3b8; font-size:13px; font-style:italic; text-align:center; padding:10px 0;">لا توجد أصناف مضافة في هذا المتجر بعد</p>`;
        } else {
            storeItems.forEach(item => {
                const itemName = item.item_name || item.name || item.title || 'صنف غير محدد';
                const itemSection = item.section || item.category || 'عام'; // لتقسيم (مقبلات، رئيسية..)
                
                itemsHtml += `
                    <div class="item-row-row">
                        <div>
                            <strong style="color:#0f172a; font-size:14px;">${itemName}</strong>
                            <span class="category-badge">${itemSection}</span>
                            <span style="color:#64748b; font-size:12px; display:block; margin-top:4px;">${item.description || ''}</span>
                        </div>
            ...
            <div class="store-card">
                <div class="store-header">
                    ...
                    <div style="display:flex; gap:10px;">
                        <button onclick="openAddItemModal('${store.id}', '${storeName}')" style="background:#eff6ff; color:#2563eb; border:1px solid #93c5fd; padding:8px 14px; border-radius:8px; font-weight:bold; cursor:pointer; font-size:13px;"><i class="fa-solid fa-plus"></i> إضافة صنف</button>
...
window.saveNewStore = async function() {
    ...
    try {
        // نرسل البيانات للحقلين المحتملين لضمان التوافق مع قاعدة بياناتك القديمة
    ...
    modal.innerHTML = `
        <div style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:9999; display:flex; justify-content:center; align-items:center; backdrop-filter: blur(4px);">
            <div style="background:#fff; width:90%; max-width:450px; border-radius:20px; padding:24px; position:relative; box-shadow:0 10px 40px rgba(0,0,0,0.2);">
                ...
                <input type="text" id="newItemName" class="form-input" placeholder="مثال: بنادول، برجر، دفتر...">
                ...
                <input type="text" id="newItemSection" class="form-input" placeholder="تصنيف الصنف داخل المتجر">
                ...
                <input type="text" id="newItemDesc" class="form-input" placeholder="وصف المنتج...">

                <button onclick="saveNewItem('${storeId}')" style="width:100%; background:var(--primary); color:white; border:none; padding:14px; border-radius:12px; font-weight:900; font-size:15px; cursor:pointer; box-shadow:0 4px 15px rgba(242,92,5,0.3);">إضافة الصنف للمتجر</button>
            </div>
        </div>
    `;
    document.body.appendChild(modal);
...
window.saveNewItem = async function(storeId) {
    ...
    try {
        const { error } = await window.supabaseClient.from('store_items').insert([{ 
            store_id: storeId, 
            name: name,
            item_name: name,
            section: section,
...
window.deleteStore = async function(storeId) {
    if(!confirm("هل أنت متأكد من حذف هذا المتجر مع جميع أصنافه التابعة له؟")) return;
    try {
