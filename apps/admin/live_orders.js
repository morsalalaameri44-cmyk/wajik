...
// 🌟 التحديث الذكي لدالة الإقفال وإضافة العهدة
window.updateOrderStatus = async function(orderId, newStatus) {
    try {
        // 1. جلب بيانات الطلب قبل التحديث للحصول على القيمة واسم المندوب
        const { data: orderData, error: orderFetchErr } = await window.supabaseClient
            .from('orders')
            .select('driver_name, total_amount, total_price, price, total')
            .eq('id', orderId)
            .single();
        ...
        // 3. إذا كان التحديث إلى "مكتمل"، قم بتحرير المندوب وإضافة العهدة
        if (newStatus === 'completed' && orderData && orderData.driver_name) {
            
            // جلب المندوب من قاعدة البيانات لمعرفة رصيد عهدته الحالي
            const { data: driverData, error: driverFetchErr } = await window.supabaseClient
                .from('drivers')
                .select('id, wallet_balance')
                .eq('driver_name', orderData.driver_name)
                .single();
                
            if (!driverFetchErr && driverData) {
                // تحديد قيمة الطلب بدقة
                const orderValue = parseFloat(orderData.total_amount || orderData.total_price || orderData.price || orderData.total) || 0;
                
                // حساب الرصيد الجديد
                const currentWallet = parseFloat(driverData.wallet_balance) || 0;
                const newWalletBalance = currentWallet + orderValue;
                
                // تحديث حالة المندوب إلى "نشط" ورفع رصيد العهدة
